...
func TestHas(t *testing.T) {
	taskDir := t.TempDir()
	loc := Loc{TaskDir: taskDir, JobID: "job1", GroupName: "group1", TaskName: "task1"}
...
func TestDir_PersistentPath(t *testing.T) {
	loc := Loc{BasePath: "/data/snapshots", TaskDir: "/alloc/task", JobID: "my-job", GroupName: "my-group", TaskName: "my-task"}
	got := loc.Dir()
	want := filepath.Join("/data/snapshots", "my-job", "my-group", "my-task")
	if got != want {
...
func TestDir_EphemeralPath(t *testing.T) {
	loc := Loc{TaskDir: "/alloc/task", JobID: "my-job", GroupName: "my-group", TaskName: "my-task"}
	got := loc.Dir()
...
func TestHas_PersistentPath(t *testing.T) {
	base := t.TempDir()
	loc := Loc{BasePath: base, JobID: "job1", GroupName: "group1", TaskName: "task1"}
	dir := loc.Dir()
	if err := os.MkdirAll(dir, 0700); err != nil {
		t.Fatal(err)
	}
