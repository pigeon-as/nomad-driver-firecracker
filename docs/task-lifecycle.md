...
2. If `snapshot_on_stop = true`, the driver pauses the VM, creates a snapshot, and saves snapshot files to the snapshot directory (`<task_dir>/snapshots/` by default, or `<snapshot_path>/<namespace>/<jobID>/<group>/<task>/` when `snapshot_path` is configured)
