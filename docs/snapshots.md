...
Snapshot files (`vmstate` and `memory`) are stored in `<task_dir>/snapshots/`. The task directory is scoped to the allocation, so snapshots naturally persist across task restarts within the same allocation (e.g. autostop/autostart) but are discarded when a new allocation is created (job update, reschedule, count change).

This is the correct behavior:
- **Same alloc restart** — config is immutable per allocation, so the snapshot is always valid
- **New allocation** — new config may differ, so cold boot is the safe default
